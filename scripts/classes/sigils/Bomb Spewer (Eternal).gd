...
#	print("Bomb Spewer recieved event: ", event, "and friendly = ", isFriendly)
	...
	if event == "card_summoned" and params[0] == card:
		for cSlot in range(4):
			if isFriendly and slotManager.is_slot_empty(slotManager.playerSlots[cSlot]) and not slotManager.is_slot_empty(slotManager.enemySlots[cSlot]):
				slotManager.summon_card(CardInfo.from_name("Explode Bot"), cSlot, true)
				print("Summoning boombot into friendly slot ", cSlot)
			
			if not isFriendly and slotManager.is_slot_empty(slotManager.enemySlots[cSlot]) and not slotManager.is_slot_empty(slotManager.playerSlots[cSlot]):
