...
if false && [ -f "$bdir/win-$vno.exe" ]; then
    ...
    7z a $bdir/win-$vno.zip $bdir/win-$vno.exe $bdir/imf_tunnel.dll $bdir/win-$vno.pck
fi
...
if false && [ -f "$bdir/linux-$vno.x86_64" ]; then
    ...
    7z a $bdir/linux-$vno.zip $bdir/linux-$vno.x86_64 $bdir/libimf_tunnel.so $bdir/linux-$vno.pck
fi
...
7z a $bdir/HTML5.zip $bdir/HTML5
